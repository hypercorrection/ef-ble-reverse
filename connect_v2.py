...
class Device:
    ...
    def __init__(self, ble_dev, adv_data):
        ...
        sn = man_data[1:17]
        if any(sn.startswith(prefix) for prefix in Device.SUPPORTED_PREFIXES):
...
class Packet:
    ...
    def __init__(self, src, dst, cmd_set, cmd_id, payload = b'', dsrc = 0, ddst = 0, version = 2, seq = None, product_id = 0):
        self._src        = src
        self._dst        = dst
    ...
    def fromBytes(data, is_xor = False):
        ...
        if payload_length > 0:
            ...
            if payload_end > len(data) - 2:
                print("ERROR: Unable to parse packet - payload truncated: " + bytearray(data).hex())
                return None
...
class Connection:
    ...
    async def getAuthStatus(self):
        ...
        packet = Packet(0x21, 0x35, 0x35, 0x89, b'', 0x01, 0x01, 0x02)
    ...
    async def enableConfigData(self, enable = True):
        ...
        packet = Packet(0x21, 0x0B, 0x0C, 0x21, payload)
    ...
    async def sendUtcTime(self):
        ...
        packet = Packet(0x21, 0x0B, 0x01, 0x55, payload)
    ...
    async def sendRTCRespond(self):
        ...
        # Forming packet
        packet = Packet(0x21, 0x35, 0x01, Packet.NET_BLE_COMMAND_CMD_SET_RET_TIME, payload, 0x01, 0x01, 0x02)
    ...
    async def sendRTCCheck(self):
        ...
        # Forming packet
        packet = Packet(0x21, 0x35, 0x01, Packet.NET_BLE_COMMAND_CMD_CHECK_RET_TIME, payload, 0x01, 0x01, 0x02)
